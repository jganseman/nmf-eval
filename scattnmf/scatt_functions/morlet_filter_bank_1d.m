...
function filters = morlet_filter_bank_1d(sig_length,options,isGPU)
	if nargin < 2
		options = struct();
	end
if(nargin<3)
isGPU=0;
	...
	for j1 = 0:options.J-1
		temp = gabor(N,psi_center(j1+1),psi_sigma(j1+1),options.precision,isGPU);%EDOUARD MODIF
		if ~do_gabor
			temp = morletify(temp,psi_sigma(j1+1),isGPU);
		end
		S = S+abs(temp).^2;
	end
	...
	for j1 = 0:length(filters.psi.filter)-1
		temp = gabor(N,psi_center(j1+1),psi_sigma(j1+1),options.precision,isGPU);
		if ~do_gabor
			temp = morletify(temp,psi_sigma(j1+1),isGPU);
		end
		filters.psi.filter{j1+1} = optimize_filter(psi_ampl*temp,0,options);
		filters.psi.meta.k(j1+1,1) = j1;
		filters.psi.meta.center(j1+1) = psi_center(j1+1);
		filters.psi.meta.bw(j1+1) = psi_sigma(j1+1);
	end
	
	% Calculate the associated low-pass filter
	if ~options.phi_dirac
		filters.phi.filter = gabor(N, 0, phi_sigma, options.precision,isGPU);
	else
...
function f = gabor(N,xi,sigma,precision,isGPU)
...
function f = morletify(f,sigma,isGPU)
	f0 = f(1);
	
	f = f-f0*gabor(length(f),0,sigma,class(f),isGPU);
if(isGPU)
    f=gpuArray(single(f));
end
