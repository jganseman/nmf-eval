% Testing divergences and powers of Alpha-NMF - a first experiment. 
% Based on the test for Beta-NMF, version 3.
...
% Using the Alpha-NMF implementation from G. Grindlay's NMFLIB.
...
highestpower = 2.5;
lowestalpha = -1.0; %-1.0
highestalpha = 2.0; %2.0;
alphainc = 0.25; %1.0;
numpowers = (highestpower - lowestpower) / powerinc +1;
numalphas = (highestalpha - lowestalpha) / alphainc +1;
mySDRs = cell([numtrials numpowers numalphas]);
mySIRs = cell([numtrials numpowers numalphas]);
mySARs = cell([numtrials numpowers numalphas]);
myPerm = cell([numtrials numpowers numalphas]);
...
for midibase = [48 72 60] % 84 36 96 54 66 42 78 90]
%for thischord= 1:10
...
for alpha = lowestalpha:alphainc:highestalpha %1
alphaindex=alphaindex+1;   

%print time
fprintf('Starting %d NMF trials, power %f and alpha %f. ', numtrials, power, alpha);
tic;
...
% Do x different NMF trials in parallel % --> PARFOR this loop
parfor trial=1:numtrials

    % perform NMFs
    % note: Grindlay's nmf_amari does NOT work for alpha=0|1 !!! Write extension?
    % --> extended grindlay's NMF to include special cases from Cichocki's NMFLab
   
    [nmfW{trial},nmfH{trial}] = nmf_amari_ext(mixstftabs, nrcomp,'niter', iter, 'alpha', alpha, 'W0', W0{trial}, 'H0', H0{trial});            
    % other options: 'thresh', [], 'norm_w', 1, 'norm_h', 0, 'verb', 1, 'myeps', 1e-20, 'W', [], 'H', []);

    %nmf_alpha_alg.m from Cichocki's NMFLab:
    %[A,X,Distance_output]=nmf_alpha_alg(Y,r,Index_norm,A_true,S,mc_on, NoAlts,restart_mc_on,AL,Y_true,type_alg_A,type_alg_X, max_restart,no_iter, alpha, omegaX, omegaA, alphaS, beta)
...
parfor trial=1:numtrials
    ...
    [mySDRs{trial, powerindex, alphaindex}, mySIRs{trial, powerindex, alphaindex}, mySARs{trial, powerindex, alphaindex}, myPerm{trial, powerindex, alphaindex}] =  bss_eval_sources( cat(1, nmfrec{trial, :}), cell2mat(arpegsrc'));
...
alphaindex=alphaindex+1;    
    ...
    mySDRmean{powerindex, alphaindex} = mean( cat(2, mySDRs{:, powerindex, alphaindex}) , 2);
    mySIRmean{powerindex, alphaindex} = mean( cat(2, mySIRs{:, powerindex, alphaindex}) , 2);
    mySARmean{powerindex, alphaindex} = mean( cat(2, mySARs{:, powerindex, alphaindex}) , 2);
    % medians
    mySDRmedian{powerindex, alphaindex} = median( cat(2, mySDRs{:, powerindex, alphaindex}) , 2);
    mySIRmedian{powerindex, alphaindex} = median( cat(2, mySIRs{:, powerindex, alphaindex}) , 2);
    mySARmedian{powerindex, alphaindex} = median( cat(2, mySARs{:, powerindex, alphaindex}) , 2);
    % stddevs
    mySDRstddev{powerindex, alphaindex} = std( cat(2, mySDRs{:, powerindex, alphaindex}) , 0, 2);
    mySIRstddev{powerindex, alphaindex} = std( cat(2, mySIRs{:, powerindex, alphaindex}) , 0, 2);
    mySARstddev{powerindex, alphaindex} = std( cat(2, mySARs{:, powerindex, alphaindex}) , 0, 2);
    % max
    mySDRmax{powerindex, alphaindex} = max( cat(2, mySDRs{:, powerindex, alphaindex}) , [], 2);
    mySIRmax{powerindex, alphaindex} = max( cat(2, mySIRs{:, powerindex, alphaindex}) , [], 2);
    mySARmax{powerindex, alphaindex} = max( cat(2, mySARs{:, powerindex, alphaindex}) , [], 2);    
    % min
    mySDRmin{powerindex, alphaindex} = min( cat(2, mySDRs{:, powerindex, alphaindex}) , [], 2);
    mySIRmin{powerindex, alphaindex} = min( cat(2, mySIRs{:, powerindex, alphaindex}) , [], 2);
    mySARmin{powerindex, alphaindex} = min( cat(2, mySARs{:, powerindex, alphaindex}) , [], 2);  
...
%% save to file
disp('Saving to file');
myfilename = ['nmf-alpha-test' int2str(thistest) '-chord' int2str(thischord) '-base' int2str(midibase) '.mat']; 
