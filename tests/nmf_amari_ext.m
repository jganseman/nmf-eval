function [W,H,errs,vout] = nmf_amari_ext(V,r,varargin)
% function [W,H,errs,vout] = nmf_amari(V,r,varargin)
%
% Implements NMF using the Amari alpha-divergence [1]:
%
%   D(V||R) = sum(V(:).^alpha .* R(:).^(1-alpha) - alpha*V(:) + ...
%                 (alpha-1)*R(:)) / (alpha*(alpha-1))
%
% This divergence reduces to the following interesting distances for
% certain values of alpha:
%
%    - Pearson's distance (alpha=2)
%    - Hellinger's distance (alpha=0.5)
%    - Neyman's chi-square distance (alpha=-1)
%
% Note that alpha should not be exactly 1 or 0.
%
...
%   alpha  [num]  - alpha parameter to the Amari-divergence [-1]
...
% Extended by Joachim Ganseman to include alpha=0 and alpha=1
% by example of Cichocki's NMFLAB's nmf_alpha_alg function
% October 2017
...
% process arguments
[alpha, niter, thresh, norm_w, norm_h, verb, myeps, W0, H0, W, H] = ...
    parse_opt(varargin, 'alpha', -1, 'niter', 100, 'thresh', [], ...
                        'norm_w', 1, 'norm_h', 0, 'verb', 1, 'myeps', 1e-20, ...
                        'W0', [], 'H0', [], 'W', [], 'H', []);

% check for bad alpha values
%if alpha == 0 || alpha == 1
%    warning('nmf_amari: alpha should not be exactly 1 or 0!');
%end
...
for t = 1:niter
    ...
    if update_W
        switch alpha        % extension by J. Ganseman
            case 0          % dual KL, updates adapted from cichocki's NMFLAB
                W = (W .* exp( log( (V+myeps)./(R+myeps)) * H' * diag(1./sum(H,2)) ));
                % note: instead of term to remove 0 values, used myeps everywhere
                % used addition diag term to normalize values into good
                % range, as in Cichocki update of A
            case 1          % KL, updates adapted from cichocki's NMFLAB
                W =  W .* ( ((V+myeps)./(R+myeps)) *H');
                % in beta_nmf, this looks like:
                %W = W .* ( ((R.^(-1) .* V)*H') ./ max(R.^(0)*H', myeps) );
            otherwise
                W = W .* ( ((V+myeps) ./ (R+myeps)).^alpha * H').^(1/alpha);
        end
        if norm_w ~= 0
            W = normalize_W(W,norm_w);
        end
    ...
    if update_H
        switch alpha        % extension by J. Ganseman
            case 0      % dual KL
                H = H .* exp(W' * (log((V+myeps)./(R+myeps))) );  %diag(1./(sum(W,1))' * 
                % note: can diag be removed? form of normalization?
                % probably, since this is transpose of updates as in NMFLAB, and diag was in W update here.
            case 1      % KL, updates adapted from cichocki's NMFLAB
                H = H .* (W' * ((V+myeps)./(R+myeps)));
                %in nmf_beta, this looks like:
                %H = H .* ( (W'*(R.^(-1) .* V)) ./ max(W'*R.^(0), myeps) );
            otherwise
                H = H .* ( (W'*((V+myeps)./(R+myeps)).^alpha) ).^(1/alpha);
        end
        if norm_h ~= 0
            H = normalize_H(H,norm_h);
        end
    ...
    errs(t) = sum(V(:).^alpha .* R(:).^(1-alpha) - alpha*V(:) + ...
                  (alpha-1)*R(:)) / (alpha*(alpha-1));
    
    % display error if asked
    if verb >= 3
        disp(['nmf_amari: iter=' num2str(t) ', err=' num2str(errs(t))]);
    end
...
% display error if asked
if verb >= 2
    disp(['nmf_amari: final_err=' num2str(errs(t))]);
end
