function [W,H,errs,iterW,iterH,logspectdist]=nmf_bp_verbose(A, opts, varargin)
% function [W,H,his,iterW,iterH,logspectdist]=nmf_bp_verbose(A, opts, varargin)
% co-author: Joachim Ganseman
%
% This function is a copy of [W,H,his] = Blockpivot(A, opts, varargin)
% from LRSlibrary, with some additional output encoded
...
% HIS (history) replaced by iterW, iterH separately; (eucl.) errs outputted.
% 
...
%function [W,H,his] = Blockpivot(A, opts, varargin)
    A = A';
    ...
    display(par);
        ...
        % CAREFUL: this routine works on the transposed!
        errs = zeros(par.max_iter,1);
        iterW = zeros(n, k, par.max_iter);     % JGA : addition
        iterH = zeros(k, m, par.max_iter);     % JGA : addition
        logspectdist = zeros(par.max_iter,1);      % JGA : addition   
...
%------------------------------------------------------------------------------------------------------------------------------
    ...
    his.error = norm(A - W*H, 'fro')^2/2.0;
    [gradW, gradH] = getGrad(A, W, H);
...
%------------------------------------------------------------------------------------------------------------------------------
        ...
        his.error = [his.error (norm(A - W*H, 'fro')^2/2.0)];
        [gradW, gradH] = getGrad(A, W, H);
        normGradW = normGrad(gradW, W);
        normGradH = normGrad(gradH, H);
        his.gradW = [his.gradW normGradH];
        his.gradH = [his.gradH normGradW];
        his.grad = [his.grad (normGradW + normGradH)];
        his.iterW = [his.iterW subIterH];
        his.iterH = [his.iterH subIterW];
        %size(HIS.gradW)
        i = iter;
        ...
        if opts.verbose
            ...
            Xr = H'*W';
            tempA = A';
            errs(iter) = sum(sum((tempA-Xr).^2)); %EUCLIDEAN!
            iterW(:,:,iter) = H';       % CAREFUL: this routine works on the transpose!
            iterH(:,:,iter) = W';
            ...
            tempdist = reshape( (10*log10((tempA(:)+eps)./(Xr(:)+eps))).^2 , n, m);
