function [W,H,errs, iterW, iterH, logspectdist] = nmf_beta_verbose(V,r,varargin)
% function [W,H,errs,iterW,iterH,logspectdist] = nmf_beta_verbose(V,r,varargin)
% co-author: Joachim Ganseman
%
% This function is a copy of [W,H,errs,vout] = nmf_beta(V,r,varargin)
% from Graham Grindlay's NMFLIB, with some additional output encoded
...
% A first version written in 2012 for my DAFX paper, 
% now adapted (not backwards compatible) in 2017 for thesis. 
%
%%%%%%%%%%
%
...
errs = zeros(niter,1);
iterW = zeros(n, r, niter);     % JGA : addition
iterH = zeros(r, m, niter);     % JGA : addition
logspectdist = zeros(niter,1);      % JGA : addition
...
for t = 1:niter
    ...
    % JGA: give W and H (current estimates) as output
    iterW(:,:,t) = W;
    iterH(:,:,t) = H;
    ...
    tempdist = reshape( (10*log10((V(:)+eps)./(R(:)+eps))).^2 , n, m);
    %TODO uncomment this to compute those values after performance testing is finished
    logspectdist(t) = mean(sqrt(sum(tempdist,1)));
