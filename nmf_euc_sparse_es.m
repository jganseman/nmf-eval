...
% function [W,H,errs,vout] = nmf_euc_sparse_es(V, r, varargin)
%
% Implements NMF using the Euclidean distance metric (see [1] for details) 
% withand the sparsity constraints proposed in [2,3]:
% 
%      min sum(sum((V-W*H).^2)) + alpha*sum(sum(H)) s.t. W>=0,H>=0
% 
...
%     Networks, 2004
% [3] M. Schmidt, J. Larsen, and F. Hsiao, "Wind Noise Reduction using
%     Non-negative Sparse Coding", IEEE MLSP, 2007.
%
...
if norm_w == 0
    warning('nmf_euc_sparse_es: really should be normalizing W!');
end
...
if norm_w ~= 2
    warning('nmf_euc_sparse_es: W should use Euclidean norm to prevent scaling drift!');
end
...
for t = 1:niter
    ...
    if update_W
        W = W .* ( (V*H' + W .* (Onn * (W*(H*H') .* W) )) ./ ...
                   max(W*(H*H') + W .* (Onn*V*H' .* W), myeps) );
